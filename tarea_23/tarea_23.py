"""
Título: Cifrar con Solitario

Enunciado:

    Solitario es un cifrado "stream", en modo "output-feedback" (salida-retroalimentación). Algunas veces se les llama
    generadores de claves ("Key-Generator", KG en la jerga militar de EE.UU.). La idea básica es que Solitario genera
    una ristra de números, llamada "keystream" (ristra o secuencia de clave), entre 1 y 26. Para cifrar, se genera una
    ristra de longitud igual al texto original. Seguidamente se suman, módulo 26, letra a letra al texto original, para
    crear el texto cifrado. Para descifrar, se genera la misma ristra y se resta, módulo 26, del texto cifrado.
    No te preocupes, explicaré qué es "módulo" en un minuto.

    Por ejemplo, para cifrar el primer mensaje mencionado en la novela de Stephenson, "DO NOT USE PC":

        Divide el mensaje original en grupos de cinco letras (No hay nada mágico respecto a los grupos de cinco letras,
        es sólo tradición). Usa "X" para completar el último grupo. Así, si el mensaje es "DO NOT USE PC", el texto se
        transformará en:
        ...
        Sumamos los números de mensaje original con los correspondientes de la ristra Solitario, módulo 26. Es decir, si
        suman más de 26, restamos 26 de resultado. Por ejemplo, 1+1=2, 26+1=27, y 27-26=1, así que 26+1=1.
    ...
    Si eres realmente bueno, puedes aprender a sumar letras en tu cabeza, y simplemente sumar las letras del paso (1) y
    (2). Sólo hace falta un poco de práctica. Es fácil recordar que A+A=B; recordar que T+Q=K es más difícil.


Comentario autor:
    No hay mucho que decir, simplemente he implementado el algoritmo. En vez de una baraja, he creado un array de 0 a 53.
    0 y 1 son los jockers. En realidad, no tenemos que generar conjuntos de números, por eso no los voy a dividir en
    grupos de 5, aunque voy a añadir las X.
"""
...
def generar_solitario(tamano, baraja):
    ...
    while (num_itera < tamano):
        ...
        index_comodin_1 = baraja.index(0)
        if index_comodin_1 == 53:
            baraja = [baraja[0]] + 0 + baraja[1:53]
        else:
            baraja[index_comodin_1] = baraja[index_comodin_1 + 1]
            baraja[index_comodin_1 + 1] = 0
        # Encuentro el comodín 2 y ejecuto paso 2: poner el comodín 2 por debajo
        index_comodin_2 = baraja.index(1)
        if index_comodin_2 == 53:
            baraja = baraja[0:2] + [1] + baraja[2:53]
        elif index_comodin_2 == 52:
            baraja = [baraja[0]] + [1] + baraja[1:52] + [baraja[53]]
        else:
            one = baraja[0:index_comodin_2]
            if type(one) != list:
                one = [one]
            two = baraja[index_comodin_2 + 1:index_comodin_2 + 3]
            three = [1]
            four = baraja[index_comodin_2 + 3:]
        ...
        index_comodin_1 = baraja.index(0)
        index_comodin_2 = baraja.index(1)
        smaller_index = min(index_comodin_1, index_comodin_2)
        higher_index = max(index_comodin_1, index_comodin_2)
        ...
        if baraja[baraja[0]] == 0 or baraja[baraja[0]] == 1:
            continue
        # Si hemos llegado aquí, obtener valor de la carta
